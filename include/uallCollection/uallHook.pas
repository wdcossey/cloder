...
begin
  ...
  //dwProcessID2 := OpenProcess(PROCESS_ALL_ACCESS,false,dwProcessID);
  dwProcessID2 := OpenProcess(PROCESS_VM_OPERATION or PROCESS_VM_READ or PROCESS_VM_WRITE, false, dwProcessID);
  if (dwProcessID2 <> 0) then
    dwProcessID := dwProcessID2;
  ...
  begin
    if WriteProcessMemory(dwProcessID,pTargetMemory,pLibraryName,dwMemSize,dwWritten) and (dwWritten = dwMemSize) then
