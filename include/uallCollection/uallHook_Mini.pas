...
begin
  ...
  begin
    if (InjectLibrary(FProcessEntry32.th32ProcessID, pLibraryName)) Then
        Inc(Result);
...
begin
  ...
  while ContinueLoop do
  begin
    if (UnloadLibrary(FProcessEntry32.th32ProcessID, pLibraryName)) then
        Inc(Result);
    ContinueLoop := Process32Next(FSnapshotHandle, FProcessEntry32);
  end;
  CloseHandle(FSnapshotHandle);
...
begin
  if (not is9x) or (is9x and (DWord(pOrigFunction) < $80000000)) then
    Result := HookCodeNt(pOrigFunction, pCallbackFunction, pnewFunction){ else
    Result := HookCodeKernel9x(pOrigFunction, pCallbackFunction, pnewFunction)};
end;

function UnhookCode(var pNewFunction: Pointer): Boolean; stdcall;
begin
  if (not is9x) or (is9x and (DWord(pNewFunction) < $80000000)) then
    Result := UnhookCodeNt(pnewFunction){ else
    Result := UnhookCodeKernel9x(pnewFunction)};
end;
